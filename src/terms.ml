...
let translate_sort info env s =
  ...
  | Type(i) -> Universes.translate_universe info env i
...
and lift_let info env x u a =
  let y = Name.fresh_identifier_of_name ~global:true ~prefix:["let"] ~default:"_" info env x in
...
and lift_fix info env names types bodies rec_indices =
  ...
  let n = Array.length names in
  let fix_names1 = Array.map (Name.fresh_identifier_of_name ~global:true ~prefix:["fix"] ~default:"_" info env) names in
  let fix_names2 = Array.map (Name.fresh_identifier_of_name ~global:true ~prefix:["fix"] ~default:"_" info env) names in
  let fix_names3 = Array.map (Name.fresh_identifier_of_name ~global:true ~prefix:["fix"] ~default:"_" info env) names in
...
(** Translate an external declaration which does not have a real type in Coq
    and push it on the info environment. *)
let translate_external info env identifier =
  (Name.push_identifier identifier env, Name.translate_identifier identifier)
